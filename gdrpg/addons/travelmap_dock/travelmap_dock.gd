...
func travelUpdatePressed(arg):
	...
	if arg[1] == mapname.text:
		travelMapList[arg[1]]["nodepath"] = String(mappath.text)
		travelMapList[arg[1]]["nodepoint"] = String(mappoint.text)
		pass
	else:
		#travelMapList[arg[1]] = null
		travelMapList.erase(arg[1])
		var sceneName = String(mapname.text)
		travelMapList[sceneName] = {"nodepath":String(mappath.text),"nodepoint":String(mappoint.text)}
		pass
	updateTravelList()
	#print(travelMapList)
	pass
